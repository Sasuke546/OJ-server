...
void _get_jobs_http(int * jobs){
	...
	sprintf(tCMD,"wget --post-data=\"getpending=1&max_running=%d\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/admin/problem_judge.php\"",max_running,http_baseurl);
	system(tCMD);
...
//	final_sleep.tv_nsec=500000000;
	...
	int j=1;
	while (j<=3){			// start to run
		int iscon=1;
	    while(iscon&&(http_judge||!init_mysql())){ 
	          iscon=work();
		}
		sleep(sleep_time);
	}
